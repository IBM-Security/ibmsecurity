...
def get_all(isamAppliance, file_path, recursive, flat_details, check_mode=False, force=False):
    """
...
def get(isamAppliance, file_path, length, size, check_mode=False, force=False):
    """
...
def clear(isamAppliance, file_id, check_mode=False, force=False):
    ...
    if force is False:
        try:
            ret_obj = get(isamAppliance, file_id, length=1, size=1)
    ...
    if force is True or (os.path.exists(filename) is False):
        if check_mode is False:  # No point downloading a file if in check_mode
            return isamAppliance.invoke_get_file(
                "Downloading a file from the file application log files area",
                "/isam/application_logs/{0}".format(file_path),
                filename)
